...
def packBits(polycube: np.ndarray) -> bytes:
    """
    Converts a 3D ndarray into a bytes object
    ...
    Returns:
    data (bytes): a bytes representation of the polycube
...
def unpackBits(cube_id: bytes) -> np.ndarray:
    """
    ...
    polycube = np.unpackbits(np.frombuffer(
        cube_id[3:], dtype=np.uint8), count=size, bitorder='little').reshape(shape)
    return polycube